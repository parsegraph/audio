export function SequenceStep(seq, i) {
  this._seq = seq;
  this._i = i;
  this._active = true;
}
...
SequenceStep.prototype.node = function () {
  ...
  const attackLabel = attackBud.spawnNode(BACKWARD, BLOCK);
  ...
  const sustainLabel = sustainBud.spawnNode(BACKWARD, BLOCK);
  sustainLabel.setLabel("Sustain", ga);
  sustainLabel.setScale(0.5);
  const sustainSliders = sustainBud.spawnNode(FORWARD, BUD);
  sustainSliders.setScale(0.5);
  let stepSlider = sustainSliders.spawnNode(FORWARD, SLIDER);
  ...
  const releaseLabel = releaseBud.spawnNode(BACKWARD, BLOCK);
...
SequencerWidget.prototype.onPlay = function (listener, listenerThisArg) {
...
SequencerWidget.prototype.node = function () {
  ...
  this._resetButton = this._recordButton.spawnNode(FORWARD, BLOCK);
  ...
  this._randomizeButton = this._resetButton.spawnNode(FORWARD, BLOCK);
  ...
  this._detuneSlider = this._randomizeButton.spawnNode(DOWNWARD, SLIDER);
  ...
  const n = car.spawn(DOWNWARD, BUD, ALIGN_CENTER);
  car.pull(DOWNWARD);
