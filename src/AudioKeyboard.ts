...
export default function AudioKeyboard(camera, ...args) {
  this._camera = camera;

  if (args.length > 1) {
    this._worldX = args[1];
    this._worldY = args[2];
    this._userScale = args[3];
  } else {
    this._worldX = 0;
    this._worldY = 0;
    this._userScale = 1;
...
AudioKeyboard.prototype.prepared = function () {
  return !!this._gl;
};

AudioKeyboard.prototype.prepare = function (gl, glyphAtlas, shaders) {
  this._gl = gl;
  this._glyphAtlas = glyphAtlas;
  this._shaders = shaders;
};

AudioKeyboard.prototype.paint = function () {
  if (!this._paintingDirty) {
    return;
  }
  const white = new Color(1, 1, 1, 1);
  const highlight = 0.2;
  const whiteBorder = new Color(1 - highlight, 1 - highlight, 1 - highlight, 1);
  const black = new Color(0, 0, 0, 1);
  const blackBorder = new Color(2 * highlight, 2 * highlight, 2 * highlight, 1);
  if (!this._whiteKeyPainter) {
    this._whiteKeyPainter = new BlockPainter(this._gl, this._shaders);
  ...
  if (!this._blackKeyPainter) {
    this._blackKeyPainter = new BlockPainter(this._gl, this._shaders);
...
AudioKeyboard.prototype.setOrigin = function (x, y) {
  this._worldX = x;
  this._worldY = y;
...
AudioKeyboard.prototype.setScale = function (scale) {
...
AudioKeyboard.prototype.render = function (world, scale) {
